...
library(dplyr)
library(tidyverse)
econ_freedom <- read_excel("~/Downloads/index2018_data.xls")
...
success_data <- tibble(econ_freedom$countryID, 
                       econ_freedom$country_name, 
                       econ_freedom$region, 
                       econ_freedom$total_score)
success_data <- success_data %>% 
  rename(countryID = `econ_freedom$countryID`, 
         country_name = `econ_freedom$country_name`, 
         region = `econ_freedom$region`, 
         econ_freedom_score = `econ_freedom$total_score`)
